...
* var_OLSest.do
*
* This STATA code estimates the no time FE coefficient matrices
...
reghdfe est_y est_y_* est_first_* est_second_* if VARsamp, absorb(cnum) res(res_y)
replace VARsamp = VARsamp & !missing(res_y)
reghdfe est_first est_y_* est_first_* est_second_* if VARsamp, absorb(cnum) res(res_first)
replace VARsamp = VARsamp & !missing(res_first)
reghdfe est_second est_y_* est_first_* est_second_* if VARsamp, absorb(cnum) res(res_second)
