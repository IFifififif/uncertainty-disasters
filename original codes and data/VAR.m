...
% VAR.m
%
% This MATLAB code computes bootstrapped SEs for
% the baseline IV-VAR in
...
probvec = [0.05 0.1 0.25 0.5 0.75 0.9 0.95 0.025 0.975];
Nprobs = length(probvec);
...
if (varct==3); SCALEFACT=sqrt(var(X(:,varct)))/Bhat(varct,varct); end

end;


Nboots = 150; %number of bootstrap replications
samplevec = (1:T)';
meanblocklength = 25;
Xorig = X;
Xmin1orig = Xmin1;
...
Bhatfullsamp = Bhat;
B1hatfullsamp = B1hat;
Dcoeffhatfullsamp = Dcoeffhat;

IRFstore = zeros(lengthIRF,NX,NX,Nboots);
Bhatstore = zeros(NX,NX,Nboots);
Dcoeffhatstore = zeros(ND,2,Nboots);


BOOTSBAD = zeros(Nboots,1);

for bootct=1:Nboots;


bootsamp = stationaryBB(samplevec,1,meanblocklength);

X = Xorig(bootsamp,:);
Xmin1 = Xmin1orig(bootsamp,:);
D = Dorig(bootsamp,:);
...
if (minOBJ> 4e-5); BOOTSBAD(bootct)=1; end;
...
Bhatstore(:,:,bootct)= Bhat;
Dcoeffhatstore(:,:,bootct) = Dcoeffhat;
...
Bhatstore = Bhatstore(:,:,BOOTSBAD==0);
IRFstore = IRFstore(:,:,:,BOOTSBAD==0);
Dcoeffhatstore = Dcoeffhatstore(:,:,BOOTSBAD==0);

IRFquantiles = zeros(lengthIRF,NX,NX,Nprobs);
IRFse = zeros(lengthIRF,NX,NX);
for ct1=1:lengthIRF;
    for ct2=1:NX;
        for ct3=1:NX;
            IRFquantiles(ct1,ct2,ct3,:) = quantile(IRFstore(ct1,ct2,ct3,:),probvec);
            IRFse(ct1,ct2,ct3)= var(IRFstore(ct1,ct2,ct3,:))^0.5;
        end;
    end;
end;


IRF_S_TO_Y_SE = IRFse(1:15,1,3);
save IV_VAR_BOOT.mat
